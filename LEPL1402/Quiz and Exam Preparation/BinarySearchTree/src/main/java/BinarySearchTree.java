...
public class BinarySearchTree {
    ...
    public List<Integer> decreasing() {
        return new ArrayList<>(decreasing(root));
    }

    public List<Integer> decreasing(Node x) {
        List<Integer> result = new ArrayList<>();
        if (x.left == null && x.right == null) { result.add(x.key); return result; }
        if (x.right != null) {
            result.addAll(decreasing(x.right));
        }
        result.add(x.key);
        if (x.left != null) {
            result.addAll(decreasing(x.left));
        }
        return result;
    }
